 ...
 */
public class BotPentominoGaps extends BotPentomino {
	/**
	 * Opens tests.
	...
	public void openParcelTests(int n, String filename) throws FileNotFoundException{
		...
		for (int i = 0; i < n; i++) {
			...
			cost = 1;
			AbstractPentominoParcel o1 = new PentominoL(cost, number);

			number = input.nextInt();
			numb2.add(number);
			cost = input.nextDouble();
			cost = 1;
			AbstractPentominoParcel o2 = new PentominoP(cost, number);
	
			number = input.nextInt();
			numb3.add(number);
			cost = input.nextDouble();
			cost = 1;
			AbstractPentominoParcel o3 = new PentominoT(cost, number);
